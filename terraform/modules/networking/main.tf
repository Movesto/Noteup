...
resource "aws_security_group" "frontend" {
  ...
  description = "Frontend ECS tasks — accepts traffic from ALB"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "backend" {
  ...
  description = "Backend ECS tasks — accepts traffic from frontend only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "RDS — accepts PostgreSQL from backend only"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "monitoring" {
  ...
  description = "Monitoring EC2 — Grafana and Prometheus access"
  vpc_id      = aws_vpc.main.id
